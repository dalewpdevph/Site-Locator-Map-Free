...
class SLM_Options_Framework_Interface {

	/**
	 * Generates the tabs that are used in the options menu
	 */
	static function slm_optionsframework_tabs() {
		$counter = 0;
	 ...
	 */
	static function slm_optionsframework_fields() {
		...
		if ( isset( $slm_optionsframework_settings['id'] ) ) {
			$option_name = $slm_optionsframework_settings['id'];
		}
		else {
			$option_name = 'slm_optionsframework';
		};
		...
		foreach ( $options as $value ) {
			...
			if ( has_filter( 'slm_optionsframework_' . $value['type'] ) ) {
				$output .= apply_filters( 'slm_optionsframework_' . $value['type'], $option_name, $value, $val );
			}
			...
			// Uploader
			case "upload":
				$output .= SLM_Options_Framework_Media_Uploader::slm_optionsframework_uploader( $value['id'], $val, null );
			...
			case 'typography':
				...
				$typography_options = array(
					'sizes' => slm_of_recognized_font_sizes(),
					'faces' => slm_of_recognized_font_faces(),
					'styles' => slm_of_recognized_font_styles(),
					'color' => true
				);
				...
				$typography_fields = apply_filters( 'slm_of_typography_fields', $typography_fields, $typography_stored, $option_name, $value );
			...
			case 'background':
				...
				$output .= SLM_Options_Framework_Media_Uploader::slm_optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
				...
				$repeats = slm_of_recognized_background_repeat();
				...
				$positions = slm_of_recognized_background_position();
				...
				$attachments = slm_of_recognized_background_attachment();
			...
			case "info":
				...
				if ( isset( $value['desc'] ) ) {
					$output .= apply_filters('slm_of_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
		...
		if ( SLM_Options_Framework_Interface::slm_optionsframework_tabs() != '' ) {
			echo '</div>';
		}
	}
